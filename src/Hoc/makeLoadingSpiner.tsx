...
function makeLoadingSinner<Props, State>(
  ...
  @observer
  class LoadingSpinnerWrapper extends React.PureComponent<
    Props & ImakeLoadingSinnerProp,
    State
  > {
    render() {
      let {type, isError, isLoading, performanceStore} = this.props;
      if (!performanceStore) {
        return <noscript />;
      }
      ...
      return (
        <div
          ...
          className={rootClazz}
        >
          <Comp {...this.props} isLoading={isLoading} />
          <LoadingSpinner
            isLoading={loading}
            isError={isError}
            onErrorHandler={this.props.scrollFunc || this.props.onErrorHandler}
          />
        </div>
      );
    }
