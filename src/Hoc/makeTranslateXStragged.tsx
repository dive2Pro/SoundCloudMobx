  ...
  class MakeTranslateXMotionWrapper extends Component<any, any> {
    ...
    componentWillReceiveProps(nextProps) {
      ...
      if (
        ...
        this.state.datas.length != nextProps.datas.length
      ) {
    ...
    getStyles = prevStyles => {
      ...
      function getDiffArr() {
        ...
        if (prevStyles.length < datas.length) {
          diffArr = datas
            .slice(prevStyles.length, datas.length)
            .map(geneStyles);
        } else {
      ...
      prevStyles = prevStyles.length > 0 && !reset
        ? getDiffArr()
        : this.state.datas.map(geneStyles);
      this.reset = false;
      return prevStyles;
    };
        ...
        : <StaggeredMotion
          ...
          >
            {(interpolatedStyles, i) => {
              return (
                <Comp {...this.props} interpolatedStyles={interpolatedStyles} />
              );
            }}
          </StaggeredMotion>;
    }
