 ...
 * @param Comp
 */
export default function makeBackToTop<Props>(
  ...
  class MakeBackToTopWrapper extends React.Component<Props, any> {
    ...
    render() {
      const style = {
        ...defaultBackToTopStyle,
        display: this.state.tagVisible ? 'block' : 'none'
      };
      return (
        <div style={defaultContainer}>
          <Comp {...this.props} />
          <em ref={n => (this.tag = n)} style={style} className={TagClass} />
