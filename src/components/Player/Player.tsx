...
class Player extends React.Component<IPlayerProps, IPlayerState> {
  ...
  @action
  setPlayerVisibleFromComponent(visible: boolean) {
  ...
  playNextTrack = () => {
    ...
    this.playNextTrack = autorun(() => {
      if (this.processValue == 1 && playerStore && !playerStore.isShuffleMode) {
  ...
  handleVolumeOpen = () => {
    const {performanceStore: pstore, playerStore} = this.props;
  ...
  renderPlayerOpearators = (store: PlayerStore) => {
    const {isPlaying, playingTrack, isShuffleMode, volume} = store;
    ...
    return (
      <div ref={n => (this.playerContainer = n)} className={styles.content}>
        ...
        <div className={styles.content_name}>
          ...
          <div className={styles.content_dur}>
            <StreamLink className={styles.trackName} id={trackId}>
              {trackName}
            </StreamLink>

            <UserLink className={styles.author} path="" id={userId}>
              {username}
            </UserLink>
          </div>
        </div>
        ...
        <div className={styles.content_plays}>
          <div alt={'random track'} className={shuffleClazz}>
            <ButtonInline onClick={this.handleShuffleMode}>
              <em className={`fa fa-random ${isUnderMediumFa}`}>&nbsp;</em>
            </ButtonInline>
          </div>

          <div alt={'next Stream'} className={styles.content_action}>
          ...
          <div className={styles.content_action}>
            <ButtonInline onClick={() => store.togglePlaying()}>
              <em
                className={`fa ${isPlaying
                  ? 'fa-pause'
                  : 'fa-play'} ${isUnderMediumFa}`}
              />
              &nbsp;
            </ButtonInline>
          </div>
          <div className={styles.content_action}>
            <ButtonInline onClick={() => this.handlePlayNext(1)}>
              <em className={`fa fa-step-forward ${isUnderMediumFa}`}>
                &nbsp;
              </em>
              &nbsp;
            </ButtonInline>
          ...
          >
            <em
              ...
              className={`fa fa-volume-${volume > 0.5
                ? 'up'
                : volume == 0 ? 'off' : 'down'} fa-2x`}
            >
              &nbsp;
            </em>
            <div
              style={{
                display: isUnderWidth
                  ? store.isVolumeOpen ? 'block' : 'none'
                  : 'block'
              }}
              ref={n => (this.volumeContainer = n)}
              className={styles.volume_container}
            >
              {(isUnderWidth && !store.isVolumeOpen) ||
                <Range
  ...
  initGlassData = () => {
    ...
    const glass = this.fronstedGlass;
