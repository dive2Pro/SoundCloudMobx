...
class PlaylistInfo extends React.PureComponent<IPlaylistInfoProp, any> {
  ...
  handleLocationChange = () => {
    const {userStore, location: {search}} = this.props;
    ...
    return (
      <div className={styles.playlistinfo}>
        ...
        <div className={styles.playlist_main}>
          <Operators tracks={tracks} isPlaylist={true} />
          <div className={styles.playlist_body}>
            <BigUserIcon
              user={user}
              handleFollow={() => {
                userStore.detectIsFollowing(user.id);
              }}
  ...
  render() {
    ...
    const isLoading =
      !userStore.fetchedPlaylist || userStore.fetchedPlaylist.id != id;
    const playlist = userStore.fetchedPlaylist;
    return (
      <div id={this.id} className={styles.playlistInfo}>
        {isLoading || !playlist
