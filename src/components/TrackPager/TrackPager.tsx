...
class TrackPager extends React.Component<ITrackPagerProps, any> {
    ...
    return (
      <form action="#" onSubmit={this.handleReplay}>
        <div className={styles.replay_div}>
          <ArtWork size={50} src={avatar_url} />
          <div className={styles.replay_input}>
            <input placeholder="Write a comment" name="replay" type="text" />
          </div>
        </div>
      </form>
    );
        ...
        <div className={styles.comments}>
          <section className={styles.replay_operators}>
            ...
            <div className={styles.operators}>
              <Operators track={currentTrack} handleLike={this.handleLike} />
            </div>
          </section>
  ...
  render() {
    const {currentTrack, isLoading, isError} = this.props.trackStore;

    return (
      <div id={this.id} className={styles.main}>
        {isLoading || !currentTrack
          ? <LoadingSpinner
              isError={isError(FETCH_TRACK)}
              onErrorHandler={() =>
                this.props.trackStore.fetchSingleTrack(this.trackId)}
              isLoading={isLoading}
            />
