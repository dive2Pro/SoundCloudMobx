...
const TabTemplate = ({children, selected, style}: any) => {
  ...
  if (!selected) {
    (templateStyle.height = 0), (templateStyle.overflow = 'hidden');
  }
  return (
...
class Tabs extends React.Component<ITabsProps, any> {
  ...
  tabs = () => {
    ...
    return this.getTabs().map((c, index) => {
      if (!c.type || c.type['uiName'] !== 'Tab') {
        console.warn(
          `Tabs only accepts  Tab component as children,${c.type} found ${index +
            1} of Tabs`
        );
      }
  ...
  getTabs = (): React.ComponentElement<any, any>[] => {
    ...
    React.Children.forEach(
      this.props.children,
      (child: React.ComponentElement<any, any>) => {
        tabs.push(child);
      }
    );
    return tabs;
