class CanvasImage {
  ...
  constructor(image) {
    ...
    this.context && this.context.drawImage(image, 0, 0, this.width, this.height);
  };
  ...
  update = (imageData) => {
    this.context && this.context.putImageData(imageData, 0, 0);
  };
...
class ColorThief {
  ...
  getColorFromUrl = (imageUrl, callback: (palettes: any[], url: string) => void, quality?) => {
...
var pv = {
  ...
  sum: function (array: any, f?): number {
    var o: any = {};
    return array.reduce(
      f
        ? function (p, d, i) {
  ...
  max: function (array: any[], f?): number {
...
var MMCQ = (function () {
  ...
  function PQueue(comparator: (a: any, b: any) => number) {
    var contents: any[] = [], sorted = false;
  ...
  CMap.prototype = {
    ...
    forcebw: function () {
      // XXX: won't  work yet
      var vboxes = this.vboxes;
      vboxes.sort(function (a: any, b: any) {
  ...
  function medianCutApply(histo, vbox): any[] | undefined {
    if (!vbox.count()) return;

    var rw: number = vbox.r2 - vbox.r1 + 1,
      gw: number = vbox.g2 - vbox.g1 + 1,
      bw: number = vbox.b2 - vbox.b1 + 1,
    ...
    var total = 0, partialsum: number[] = [], lookaheadsum: number[] = [], i, j, k, sum, index;
    if (maxw == rw) {
      for (i = vbox.r1; i <= vbox.r2; i++) {
        sum = 0;
        for (j = vbox.g1; j <= vbox.g2; j++) {
