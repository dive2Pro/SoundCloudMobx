...
import {PicSize, unauthApiUrl} from '../services/soundcloundApi';
...
export class TrackStore extends BaseStreamStore<ITrack> {
  ...
  bindDebounced = () => {
    if (!this.debouncedFetchData) {
      this.debouncedFetchData = debounce(this.fetchData.bind(this), 50).bind(
        this
      );
    }
  };
  ...
  setTrackId(id: number) {
    let track = this.currentTracks
      .filter(t => t != null)
      .find(t => t.id === id);
    if (!track) {
      track = this.allTracks().filter(t => t != null).find(t => t.id === id);
    }
    if (track) {
  ...
  @computed
  get sliderImages() {
    return this.currentTracks.slice(0, 5).map(
      createTransformer((track: ITrack) => ({
        src: getSpecPicPath(track.artwork_url, PicSize.MASTER),
        id: track.id
      }))
    );
  }
  ...
  allTracks(): ITrack[] {
    ...
    const tracks = values.length > 0
      ? values.reduce((preEntrey, currentEntry) => {
          return preEntrey.concat(currentEntry);
        })
      : [];
    return activitiesStore.tracks.concat(
      tracks,
      UserStore.AllUsersFavorities()
    );
  }
  ...
  @action
  async fetchTracks(urlCallback?: () => string) {
    let requestGenre = this.currentGenre.toLocaleLowerCase() || 'country',
      url;
    url = this.nextHref;
  ...
  @action
  setGenre(genre: string, urlCallback?: () => string) {
