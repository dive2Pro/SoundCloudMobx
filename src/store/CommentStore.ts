...
export class CommentStore {
  ...
  @computed
  get currentCommentNextHref(): string {
    return (
      (this.currentTrack &&
        this.nextHrefsByTrack.get(this.currentTrack.id + '')) ||
      ''
    );
  }
  ...
  @action
  async fetchMoreComments(nextHref?: string) {
    ...
    let url = nextHref
      ? nextHref
      : apiUrl(
          `tracks/${id}/comments?linked_partitioning=1&limit=50&offset=0`,
          '&'
        );
