...
import {ISession} from '../interfaces/interface';
...
export class SessionStore {
  ...
  // TODO : type
  @action
  catchError({err, fetchType}: ICatchErr) {
    // console.error(err);
    throw err;
  }
  @action
  login() {
    if (this.loadDataFromCookie()) {
      return;
    }
    ...
    SC.connect()
      .then((session: ISession) => {
        ...
        this.oauthToken = session.oauth_token;
    ...
    const rawUser: any = {
      ...
      description: 'know when I get rejec',
      city: null,
      discogs_name: null,
      myspace_name: null,
      website: null,
