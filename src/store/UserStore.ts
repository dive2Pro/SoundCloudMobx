...
import {IPlaylist, IStream, ITrack, IMiniUser} from '../interfaces/interface';
import {
  addAccessToken,
  apiUrl,
  unauthApiUrlV2,
...
export class UserStore {
  ...
  @action
  setCurrentUserModel(model: UserModel) {
    this.userModel = model;
  }

  @computed
  get isLoginUser() {
    if (this.loginedUserId == null) {
      return false;
    }
  ...
  @computed
  get getLoginUserModel(): UserModel | undefined {
  ...
  @action
  setFetchedPlaylistInfo = (data: IPlaylist | undefined) => {
  ...
  @action
  findPlaylistFromCurrentUser = (id: number) => {
  ...
  async detectIsFollowing(id: number) {
    ...
    const data = await fetch(
      unauthApiUrlV2(
        `users/${id}/followings/not_followed_by/${this.loginedUserId}`,
        '?'
      )
    );
  }
  ...
  isTrackLiked = (track: ITrack): boolean => {
    if (!!this.getLoginUserModel) {
      return this.getLoginUserModel.favorites.some(item =>
        isEqual(item, track)
      );
    }
    return false;
  ...
  private async followUser(user: User) {
    ...
    const data: any = await fetch(apiUrl(`me/followings/${id}`, '?'), {
  ...
  @action
  private operaUserFromFollowings(user: User, followed: boolean) {
...
export class User implements IMiniUser {
  ...
  @action
  updateFromServe = (rawUser: User) => {
    extendObservable(this, rawUser);
  };

  @action
  setFollowingState(following: boolean) {
...
export class UserModel {
  ...
  constructor(userStore: UserStore, obj: number | User) {
    ...
    when(
      () => userStore.isLogined,
      () => this.followingFilterByLogin(userStore.isLogined),
      this
    );
  }
  ...
  getAllTrackFromStreams(): ITrack[] {
    return this.streams
      .filter(stream => stream.track != null)
      .map(s => s.track);
  }

  fetchCommunityDataWithEmptyLength() {
    [FETCH_FOLLOWERS, FETCH_FAVORITES, FETCH_FOLLOWINGS].forEach(item => {
  ...
  @action
  changeLoadingState(type: string, loading: boolean) {
    this.isLoadings.set(type, loading);
  }

  @action
  changeNextHrefs(type: string, nextHref: string) {
    this.nextHrefs.set(type, nextHref);
  }

  @action
  resetLoadingState(type: string) {
    this.isLoadings.set(type, false);
  }

  @action
  addData(type: string, fs: any[]) {
  ...
  @action
  async followingFilterByLogin(isLogined: boolean) {
    if (isLogined) {
      this.followers.forEach(
        user => (user.isFollowing = this.userStore.isFollowingUser(user.id))
      );
      this.followings.forEach(
        user => (user.isFollowing = this.userStore.isFollowingUser(user.id))
      );
    }
  }
  ...
  @action
  async fetchWithType(type: string) {
    ...
    if (url) {
      ...
      switch (fetchType) {
        ...
        case FETCH_PLAYLIST:
          ...
          url = unauthApiUrl(
            url + `?limit=${limitPageSize}&offset=0&linked_partitioning=1`,
            '&'
          );
          break;
        default:
